...
pub struct RunOptions {
    ...
    /// JVM build support: run the proxy tokenless and inject JVM proxy
    /// system properties (JAVA_TOOL_OPTIONS) plus generated maven/gradle
...
/// intra-sandbox services must not be detoured through the proxy.
fn jvm_proxy_props(proxy_host: &str, proxy_port: u16) -> String {
    // Single line, space separated: JAVA_TOOL_OPTIONS is split on whitespace,
    // so embedded continuations would leak stray tokens into every JVM.
    format!(
        "-Dhttp.proxyHost={h} -Dhttp.proxyPort={p} -Dhttps.proxyHost={h} \
         -Dhttps.proxyPort={p} -Dhttp.nonProxyHosts=localhost|127.*|[::1]",
        h = proxy_host,
        p = proxy_port
    )
}

/// Minimal Maven settings.xml wiring all protocols through the loopback proxy.
/// Only written when the user has no settings.xml of their own — an existing
/// file (corp mirrors, Nexus servers, their own <proxies>) always wins.
fn maven_settings_xml(proxy_host: &str, proxy_port: u16) -> String {
    format!(
        r#"<?xml version="1.0" encoding="UTF-8"?>
<!-- Generated by agentbox (java-proxy-config). Absent on disk => your own
     ~/.m2/settings.xml was kept as-is. -->
<settings xmlns="http://maven.apache.org/SETTINGS/1.0.0">
  <proxies>
    <proxy>
      <id>agentbox-http</id><active>true</active><protocol>http</protocol>
      <host>{h}</host><port>{p}</port><nonProxyHosts>localhost|127.*</nonProxyHosts>
    </proxy>
    <proxy>
      <id>agentbox-https</id><active>true</active><protocol>https</protocol>
      <host>{h}</host><port>{p}</port><nonProxyHosts>localhost|127.*</nonProxyHosts>
    </proxy>
  </proxies>
</settings>
...
/// Gradle proxy properties. Gradle cannot authenticate against proxies via
/// gradle.properties at all, so this pairs with the tokenless proxy mode.
fn gradle_properties(proxy_host: &str, proxy_port: u16) -> String {
    format!(
        "# Generated by agentbox (java-proxy-config).
\
         systemProp.http.proxyHost={h}\n\
         systemProp.http.proxyPort={p}\n\
         systemProp.https.proxyHost={h}\n\
         systemProp.https.proxyPort={p}\n\
...
/// Write generated JVM tool configs into scratch HOME unless a snapshotted
/// user file already occupies the path.
fn generate_java_configs(scratch_home: &Path, proxy_host: &str, proxy_port: u16) {
    let targets: &[(&str, String)] = &[
        (
            ".m2/settings.xml",
            maven_settings_xml(proxy_host, proxy_port),
        ),
        (
            ".gradle/gradle.properties",
            gradle_properties(proxy_host, proxy_port),
        ),
    ];
    for (rel, content) in targets {
        let dest = scratch_home.join(rel);
        if dest.exists() {
            continue; // user's own config wins
        }
        if let Some(parent) = dest.parent() {
            let _ = std::fs::create_dir_all(parent);
        }
        match std::fs::write(&dest, content) {
            Ok(_) => eprintln!("agentbox: generated ~/{rel} (java-proxy-config)", rel = rel),
            Err(e) => {
                eprintln!("agentbox: warning: ~/{rel} not generated: {e}", rel = rel)
            }
        }
    }
}
...
fn build_env(
    ...
    // per property).
    let mut jto = format!("-Djava.io.tmpdir=\"{}\"", scratch_tmp.display());
    if let Some(props) = &jvm_props {
        jto.push(' ');
        jto.push_str(props);
    }
    if let Ok(v) = std::env::var("JAVA_TOOL_OPTIONS") {
        if !v.trim().is_empty() {
            jto.push(' ');
            jto.push_str(v.trim());
        }
...
pub async fn run_session(opts: RunOptions) -> anyhow::Result<SessionOutcome> {
    ...
    let (bound, proxy_url, proxy_port): (Option<ab_proxy::BoundProxy>, Option<String>, Option<u16>) =
        ...
        let token = if macos && !opts.java_proxy_config {
            Some(ab_proxy::generate_token())
        } else {
            if opts.java_proxy_config {
                eprintln!(
                    "agentbox: java-proxy-config is active — proxy runs TOKENLESS so JVM \
                     system properties can use it. Other local processes could borrow this \
                     session's allowlist on the same random port; acceptable for single-user \
                     dev machines, not for shared hosts."
                );
            }
            None
        };
        ...
        (Some(bound), Some(url), Some(port))
    };

    // ---- JVM proxy config (java-proxy-config) -----------------------------
    let jvm_props = match (proxy_port, opts.java_proxy_config) {
        (Some(port), true) => {
            // Tokenless mode: the loopback endpoint is the bare host:port.
            generate_java_configs(&scratch_home, "127.0.0.1", port);
            Some(jvm_proxy_props("127.0.0.1", port))
        }
        _ => None,
    };
...
mod tests {
    ...
    fn jvm_props_include_both_schemes_and_nonproxy() {
        let props = jvm_proxy_props("127.0.0.1", 45678);
        assert!(props.contains("-Dhttp.proxyHost=127.0.0.1"));
        assert!(props.contains("-Dhttp.proxyPort=45678"));
        assert!(props.contains("-Dhttps.proxyHost=127.0.0.1"));
        assert!(props.contains("-Dhttps.proxyPort=45678"));
        assert!(props.contains("nonProxyHosts=localhost|127.*|[::1]"));
    }

    #[test]
    fn maven_settings_wires_http_and_https_proxies() {
        let xml = maven_settings_xml("127.0.0.1", 40001);
        assert!(xml.contains("<id>agentbox-http</id>"));
        assert!(xml.contains("<protocol>https</protocol>"));
        assert!(xml.contains("<port>40001</port>"));
        assert!(xml.contains("nonProxyHosts"));
    }

    #[test]
    fn gradle_properties_covers_both_schemes() {
        let gp = gradle_properties("127.0.0.1", 40002);
        assert!(gp.contains("systemProp.http.proxyPort=40002"));
        assert!(gp.contains("systemProp.https.proxyHost=127.0.0.1"));
    }

    #[test]
    fn generate_skips_existing_user_config() {
        let dir = tempfile::tempdir().unwrap();
        let existing = dir.path().join(".m2");
        std::fs::create_dir_all(&existing).unwrap();
        std::fs::write(existing.join("settings.xml"), "<settings/>").unwrap();
        generate_java_configs(dir.path(), "127.0.0.1", 1);
        // User's own settings.xml must be untouched.
        let content =
            std::fs::read_to_string(existing.join("settings.xml")).expect("user file kept");
        assert_eq!(content, "<settings/>");
        // gradle.properties absent => generated.
        let gp = std::fs::read_to_string(dir.path().join(".gradle/gradle.properties"))
            .expect("gradle.properties generated");
        assert!(gp.contains("systemProp.http.proxyHost=127.0.0.1"));
    }
}
