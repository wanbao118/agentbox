...
mod tests {
    ...
    fn protected_ranges_are_rejected() {
        ...
        assert!(is_protected_destination("::ffff:127.0.0.1".parse().unwrap()));
        assert!(is_protected_destination("::ffff:10.0.0.7".parse().unwrap()));
        assert!(is_protected_destination("::ffff:169.254.169.254".parse().unwrap()));
    }

    #[test]
    fn public_addresses_pass() {
        for s in ["1.2.3.4", "8.8.8.8", "52.84.1.2", "172.32.0.1", "100.128.0.1", "9.9.9.9"] {
